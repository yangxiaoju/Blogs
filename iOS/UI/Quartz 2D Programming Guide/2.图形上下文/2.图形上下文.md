...
表2-1总结了位图图形上下文支持的像素格式，关联的色彩空间（cs）以及格式首次可用的 Mac OS X 版本。像素格式被指定为每像素位数（bpp）和每个分量位数（bpc）。该表还包括与该像素格式相关联的位图信息常数。有关每个位图信息格式常量所代表的内容的详细信息，请参阅 CGImage Reference。
